...
constexpr wchar_t kPaneStockColorsToken[] = L"pane_use_stock_colors";
...
bool OptionsStore::Load() {
    ...
    ParseConfigLines(content, kCommentChar, L'|', [&](const std::vector<std::wstring>& tokens) {
        ...
        if (header == kPaneStockColorsToken) {
            if (tokens.size() >= 2) {
                m_options.useStockExplorerPaneColors = ParseBool(tokens[1]);
            }
            return true;
        }
...
bool OptionsStore::Save() const {
    ...
    content += kPaneStockColorsToken;
    content += L"|";
    content += m_options.useStockExplorerPaneColors ? L"1" : L"0";
    content += L"\n";
...
bool operator==(const ShellTabsOptions& left, const ShellTabsOptions& right) noexcept {
    return left.reopenOnCrash == right.reopenOnCrash && left.persistGroupPaths == right.persistGroupPaths &&
           ...
           left.useStockExplorerPaneColors == right.useStockExplorerPaneColors &&
