...
struct ShellTabsOptions;
	...
	private:
		...
		void ScheduleEnsureRetry(HRESULT lastError);
		void CancelEnsureRetryTimer();
		void OnEnsureRetryTimer();
		static void CALLBACK EnsureRetryTimerProc(HWND hwnd, UINT msg, UINT_PTR id, DWORD time);
		void MaybeShowAutomationDisabledToast(HRESULT hr);
		...
		UINT m_ensureRetryAttempt = 0;
		UINT_PTR m_ensureRetryTimerId = 0;
		bool m_automationPolicyToastShown = false;
